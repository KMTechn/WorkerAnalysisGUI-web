...
class WorkerAnalysisGUI:
    ...
    def _draw_overall_comparison_tab(self, parent):
        ...
        standby_trays = transfer_kpis.get('total_trays', 0) - packaging_kpis.get('total_trays', 0)
        standby_pcs = transfer_kpis.get('total_pcs_completed', 0) - packaging_kpis.get('total_pcs_completed', 0)
        comparison_data = [
            ('총 처리 세트 (Tray)', f"{transfer_kpis.get('total_trays', 0):,} 개", f"{packaging_kpis.get('total_trays', 0):,} 개", f"{standby_trays:,} 개"),
            ('총 처리 수량 (PCS)', f"{transfer_kpis.get('total_pcs_completed', 0):,} 개", f"{packaging_kpis.get('total_pcs_completed', 0):,} 개", f"{standby_pcs:,} 개"),
            ('평균 작업 시간', self._format_seconds(transfer_kpis.get('avg_tray_time', 0)), self._format_seconds(packaging_kpis.get('avg_tray_time', 0)), '—'),
            ('평균 준비 시간', self._format_seconds(transfer_kpis.get('avg_latency', 0)), self._format_seconds(packaging_kpis.get('avg_latency', 0)), '—'),
            ('초도 수율 (FPY)', f"{transfer_kpis.get('avg_fpy', 0):.1%}", f"{packaging_kpis.get('avg_fpy', 0):.1%}", '—'),
            ('총 공정 오류 수', f"{transfer_kpis.get('total_errors', 0):,} 건", f"{packaging_kpis.get('total_errors', 0):,} 건", '—')
        ]
        ...
        columns_config = {
            ...
            'transfer': {'text': '이적실 (완료)', 'anchor': 'center'},
            'packaging': {'text': '포장실 (완료)', 'anchor': 'center'},
            'standby': {'text': '포장 대기', 'anchor': 'center'}
        }
        self._setup_treeview_columns(tree, columns_config, 'comparison_table', stretch_col='metric')
        for i, (metric, t_val, p_val, s_val) in enumerate(comparison_data):
            tag = "RedRow.Treeview" if '대기' in s_val and standby_trays > 0 else ("oddrow" if i % 2 else "")
            tree.insert('', 'end', values=(metric, t_val, p_val, s_val), tags=(tag,))
        ...
        self.paned_windows['comparison_h_pane'] = charts_h_pane
        transfer_chart_frame = ttk.Frame(charts_h_pane)
        charts_h_pane.add(transfer_chart_frame, weight=1)
        packaging_chart_frame = ttk.Frame(charts_h_pane)
        charts_h_pane.add(packaging_chart_frame, weight=1)
    ...
    def _draw_daily_production_chart(self,parent, df_to_use, title, period_type="D"):
        ...
        ax.grid(True,which='both',linestyle='--',linewidth=0.5,zorder=0); ax.set_facecolor(self.COLOR_SIDEBAR_BG); fig.tight_layout(pad=2.0)
    ...
    def _draw_speed_accuracy_scatter(self,parent):
        ...
        if df is not None and not df.empty and 'avg_work_time_norm' in df.columns and 'first_pass_yield_norm' in df.columns:
            ...
            if pd.notna(mean_x) and pd.notna(mean_y):
                ...
                ax.text(float(mean_x), ax.get_ylim()[1], "정확성↑", ha='center', va='top', color='gray')
                ax.text(ax.get_xlim()[1], float(mean_y), "신속성→", ha='right', va='center', color='gray')
    ...
    def _draw_realtime_tab_content(self, parent_tab):
        ...
        if mode == "이적실":
            ...
            main_v_pane.pack(fill=tk.BOTH, expand=True)
            self.paned_windows[pane_name_v] = main_v_pane
            
            top_frame = ttk.Frame(main_v_pane, style='TFrame')
            main_v_pane.add(top_frame, weight=1)

            pane_name_h = "realtime_packaging_h_top"
            top_h_pane = ttk.PanedWindow(top_frame, orient=tk.HORIZONTAL)
            top_h_pane.pack(fill=tk.BOTH, expand=True)
            self.paned_windows[pane_name_h] = top_h_pane
            
            item_frame = ttk.Frame(top_h_pane, style='Card.TFrame', padding=20)
            top_h_pane.add(item_frame, weight=1)
            hourly_chart_frame = ttk.Frame(top_h_pane)
            top_h_pane.add(hourly_chart_frame, weight=1)
            
            self._draw_realtime_item_status(item_frame, pd.DataFrame())
            self._draw_realtime_hourly_production_chart(hourly_chart_frame, pd.DataFrame(), "포장실 시간별 생산량 (오늘 vs 평균)", pd.DataFrame())
            
            bottom_frame = ttk.Frame(main_v_pane, style='Card.TFrame', padding=(20, 10, 20, 20))
            ...
            if mode == "이적실":
                if 'realtime_main' in self.paned_windows and self.paned_windows['realtime_main'].winfo_exists():
                    ...
                    if main_pane.winfo_children() and main_pane.winfo_children()[0].winfo_children():
                        ...
                        self._draw_realtime_hourly_production_chart(hourly_chart_frame, self.realtime_today_df, "이적실 시간별 생산량 (오늘 vs 평균)", self.filtered_df_raw)
                        self._draw_realtime_item_status(item_frame, self.realtime_today_df)
            else: # mode == "포장실"
                if 'realtime_packaging_v_main' in self.paned_windows and self.paned_windows['realtime_packaging_v_main'].winfo_exists():
                    main_v_pane = self.paned_windows['realtime_packaging_v_main']
                    if main_v_pane.winfo_children():
                        top_frame = main_v_pane.winfo_children()[0]
                        if top_frame.winfo_children():
                            top_h_pane = top_frame.winfo_children()[0]
                            if top_h_pane.winfo_children():
                                item_frame = top_h_pane.winfo_children()[0]
                                hourly_chart_frame = top_h_pane.winfo_children()[1]
                                self._draw_realtime_item_status(item_frame, self.realtime_today_df)
                                self._draw_realtime_hourly_production_chart(hourly_chart_frame, self.realtime_today_df, "포장실 시간별 생산량 (오늘 vs 평균)", self.filtered_df_raw)
    ...
    def _on_realtime_worker_double_click(self, event):
        ...
        if not item_values or item_values[0] in ["총계", ""]:
            return
